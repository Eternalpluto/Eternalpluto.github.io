...
// 获取当前页面类型的副标题
- let pageSubContent = subContent
- if ((globalPageType === 'tag' || globalPageType === 'tags') && theme.subtitle.tag_sub) pageSubContent = theme.subtitle.tag_sub
- else if ((globalPageType === 'category' || globalPageType === 'categories') && theme.subtitle.category_sub) pageSubContent = theme.subtitle.category_sub
- else if (globalPageType === 'archive' && theme.subtitle.archive_sub) pageSubContent = theme.subtitle.archive_sub
...
script.
  window.typedJSFn = {
    ...
    processSubtitle: (content, extraContents = []) => {
      if (!{effect}) {
        const sub = !{JSON.stringify(pageSubContent)}.slice()
...
case source
  ...
  default
    if pageSubContent.length > 0
      script.
        function subtitleType () {
          typedJSFn.processSubtitle(!{JSON.stringify(pageSubContent)})
        }
        typedJSFn.run(subtitleType)